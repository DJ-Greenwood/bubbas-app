...
exports.processEmotionalChat = exports.continueConversation = exports.startEmotionalSupportSession = exports.analyzeEmotionWithTracking = exports.analyzeEmotion = exports.callOpenAI = void 0;
// src/callOpenAI.ts
 ...
 */
exports.callOpenAI = functions.onCall({
    ...
    if (!auth && userId) {
        ...
        throw new functions.HttpsError('unauthenticated', 'Authentication required to specify userId');
    }
    ...
    if (!messages || !Array.isArray(messages) || messages.length === 0) {
        throw new functions.HttpsError('invalid-argument', 'Missing or invalid "messages" array');
    }
    ...
    try {
        ...
        if (saveHistory && effectiveUserId && sessionId) {
            await saveConversationHistory(effectiveUserId, sessionId, typedMessages[typedMessages.length - 1], { role: "assistant", content: content }, selectedModel, transactionId);
        }
        return {
            content,
            model: selectedModel,
        ...
        throw new functions.HttpsError('internal', 'Failed to fetch response from OpenAI');
    }
});
/**
 * Cloud Function to analyze the emotional content of a text
 */
exports.analyzeEmotion = functions.onCall({
    secrets: [OPENAI_API_KEY, OPENAI_MODEL],
}, async (request) => {
    ...
    // Validate text input
    if (!text) {
        throw new functions.HttpsError('invalid-argument', 'Text input is required');
    }
    // Get API key and default model
    const apiKey = OPENAI_API_KEY.value();
 ...
 */
exports.analyzeEmotionWithTracking = functions.onCall({
    secrets: [OPENAI_API_KEY, OPENAI_MODEL],
}, async (request) => {
    ...
    try {
        ...
        try {
            ...
            throw new functions.HttpsError('internal', "Failed to parse emotion analysis response");
        }
    }
    catch (error) {
 ...
 */
exports.startEmotionalSupportSession = functions.onCall({
    secrets: [OPENAI_API_KEY, OPENAI_MODEL],
}, async (request) => {
 ...
 */
exports.continueConversation = functions.onCall({
    secrets: [OPENAI_API_KEY, OPENAI_MODEL],
}, async (request) => {
    ...
    if (!sessionId) {
        throw new functions.HttpsError('invalid-argument', "Session ID is required");
    }
    if (!message) {
 ...
 */
exports.processEmotionalChat = functions.onCall({
    secrets: [OPENAI_API_KEY, OPENAI_MODEL],
}, async (request) => {
