...
import { redirectToCheckout, redirectToCustomerPortal, cancelCurrentSubscription, formatCurrency } from './stripeService';
...
export const getUserTier = async (): Promise<SubscriptionData> => {
  try {
    ...
    // Add subscription dates and Stripe data from user data
    return {
      ...tierData,
      tier,
      ...
      stripeProductId: userData.subscription?.stripeProductId || ''
    };
  } catch (error) {
...
// Update the user's subscription tier (manual update without payment)
...
// Subscribe to a paid tier with Stripe
export const subscribeWithStripe = async (tier: 'plus' | 'pro'): Promise<void> => {
  // This will redirect to Stripe Checkout
  await redirectToCheckout(tier);
};

// Manage the subscription in Stripe Customer Portal
...
export const cancelSubscription = async (): Promise<boolean> => {
  return await cancelCurrentSubscription();
};
...
export const isTrialActive = (subscription: SubscriptionData): boolean => {
  return subscription.status === 'trial' && getDaysRemaining(subscription.trialEnds) > 0;
};

// Get trial days remaining
export const getTrialDaysRemaining = (subscription: SubscriptionData): number => {
  return subscription.status === 'trial' ? getDaysRemaining(subscription.trialEnds) : 0;
};